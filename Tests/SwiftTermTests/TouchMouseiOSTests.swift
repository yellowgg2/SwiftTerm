...
struct TouchMouseiOSTests {
    ...
    /// 새 public API default는 automatic이고 callback은 DECSET/DECRST availability만 전달한다.
    @Test
    ...
    /// local은 native scrollback owner를 남기고 scrollWheel은 one-finger recognizer만 설치한다.
    @Test
    ...
    func activeSelectionPreventsTouchWheelPanFromBeginning() throws {
        ...
        sut.withTerminal { _ in sut.selection.active = true }
        #expect(!sut.gestureRecognizerShouldBegin(pan))

        sut.withTerminal { _ in sut.selection.active = false }
        #expect(sut.gestureRecognizerShouldBegin(pan))
    }
    ...
    /// policy, remote availability, command reset, UI shutdown은 pending wheel state를 폐기한다.
    @Test
    func policyAvailabilityResetAndShutdownClearPendingWheelState() {
        let (sut, _) = makeSUT()
    ...
    /// horizontal movement와 two-finger pinch arbitration은 wheel byte를 만들지 않는다.
    @Test
    ...
    /// host double-tap handler는 recognizer callback에서 한 번 소비되고 remote byte를 만들지 않는다.
    @Test
    ...
    /// wheel adapter는 negotiated encoder를 사용해 natural swipe-up을 button 5로 전송한다.
    @Test
